...
fn process_claim_yield(e: &Env, adapter_address: &Address, supported_assets: Vec<Address>) -> i128 {
    ...
    for asset in supported_assets.iter() {
        ...
        if let Some((id, fn_name, args)) = adapter.claim_yield_auth(&asset, &e.current_contract_address()) {
            utils::authenticate_contract(
                &e, 
                id, 
                fn_name,  
        ...
        let claimed = adapter.claim_yield(&asset, &e.current_contract_address());
        
        if claimed > 0 {

            let deposited = process_deposit_for_claim(e, adapter_address.clone(), e.current_contract_address(), asset.clone(), claimed);
...
pub fn process_deposit_for_claim(e: &Env, protocol_id: Address, user: Address, asset: Address, amount: i128) -> i128 { 
